...
class Auth extends CI_Controller
{
	...
	public function create_user()
	{
		...
		if ($this->form_validation->run() == true) {
			...
			$additional_data = array(
				...
				'uploader'   => $this->session->userdata('user_id') // Tetap menyimpan ID user yang menginput data
			);
				...
				if ($this->ion_auth->update($user->id, $data)) {
					...
					$this->session->set_flashdata('message', '
						<div class="alert alert-block alert-danger">
							...
							<i class="ace-icon fa fa-bullhorn red"></i> Update Data Gagal
						</div>');
					redirect(site_url('admin/auth/'));
				}
			}
	...
	public function delete_user($id)
	{
		...
		if ($this->Ion_auth_model->delete_user($id)) {
			...
			$this->session->set_flashdata('message', '<div class="alert alert-danger">Gagal menghapus user.</div>');
		}
	
		redirect('admin/auth'); // Pastikan ini benar-benar halaman index
	}
